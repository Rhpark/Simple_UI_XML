...
/**
 * RelativeLayout that uses ViewBinding with a provided inflate function.<br><br>
 * 제공된 inflate 함수로 ViewBinding을 사용하는 RelativeLayout입니다.<br>
 *
 * **Why this class exists / 이 클래스가 필요한 이유:**<br>
 * - Provides lifecycle-aware ViewBinding for custom RelativeLayout components.<br>
 * - Ideal for legacy layouts or components requiring relative positioning of child views.<br>
 * - Automatically initializes binding on attach and cleans up on detach.<br><br>
 * - 커스텀 RelativeLayout 컴포넌트에 생명주기 인식 ViewBinding을 제공합니다.<br>
 * - 레거시 레이아웃이나 자식 뷰의 상대적 위치 지정이 필요한 컴포넌트에 적합합니다.<br>
 ...
 * ) : BaseViewBindingRelativeLayout<LayoutMediaControlBinding>(
 *     context, attrs,
 *     inflate = LayoutMediaControlBinding::inflate,
 * ) {
 *     override fun onInitBind(binding: LayoutMediaControlBinding) {
 *         binding.btnPlay.setOnClickListener { /* play action */ }
 *         binding.btnPause.setOnClickListener { /* pause action */ }
 *         binding.seekBar.setOnSeekBarChangeListener(/* ... */)
 *     }
 *
 *     fun setProgress(progress: Int) {
 *         getBinding().seekBar.progress = progress
 *     }
 * }
 *
 * // 2. Use in XML
 * // <com.example.MediaControlView
