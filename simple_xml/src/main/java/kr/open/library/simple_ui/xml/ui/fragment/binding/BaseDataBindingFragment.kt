 ...
 * - Does NOT automatically set lifecycleOwner - subclasses must set it manually in onViewCreated() if needed for LiveData observation.<br>
 ...
 * - lifecycleOwner를 자동으로 설정하지 않음 - LiveData 관찰이 필요한 경우 하위 클래스에서 onViewCreated()에서 수동으로 설정해야 합니다.<br>
 ...
 * 2. Set lifecycleOwner in onViewCreated() if you need LiveData observation: `binding.lifecycleOwner = viewLifecycleOwner`<br>
 ...
 * 2. LiveData 관찰이 필요한 경우 onViewCreated()에서 lifecycleOwner를 설정하세요: `binding.lifecycleOwner = viewLifecycleOwner`<br>
 ...
 *         binding.lifecycleOwner = viewLifecycleOwner  // Required for LiveData observation
 *     }
 *
...
public abstract class BaseDataBindingFragment<BINDING : ViewDataBinding>(
     ...
     * Note: lifecycleOwner is NOT automatically set - set it manually in onViewCreated() if needed.<br><br>
     * DataBindingUtil을 사용하여 DataBinding 인스턴스를 생성합니다.<br>
     * 참고: lifecycleOwner는 자동으로 설정되지 않음 - 필요한 경우 onViewCreated()에서 수동으로 설정하세요.<br>
     *
