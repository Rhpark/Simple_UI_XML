...
/**
 * ConstraintLayout that uses ViewBinding with a provided inflate function.<br><br>
 * 제공된 inflate 함수로 ViewBinding을 사용하는 ConstraintLayout입니다.<br>
 *
 * **Why this class exists / 이 클래스가 필요한 이유:**<br>
 * - Provides lifecycle-aware ViewBinding for custom ConstraintLayout components.<br>
 * - Automatically initializes binding on attach and cleans up on detach.<br>
 * - Supports ViewModel event collection through onEventVmCollect().<br><br>
 * - 커스텀 ConstraintLayout 컴포넌트에 생명주기 인식 ViewBinding을 제공합니다.<br>
 * - attach 시 자동으로 바인딩을 초기화하고 detach 시 정리합니다.<br>
 * - onEventVmCollect()를 통해 ViewModel 이벤트 수집을 지원합니다.<br>
 *
 * **Usage / 사용법:**<br>
 ...
 * ) : BaseViewBindingConstraintLayout<LayoutProfileCardBinding>(
 *     context, attrs,
 *     inflate = LayoutProfileCardBinding::inflate,
 * ) {
 *     private val viewModel: ProfileViewModel by lazy { /* get ViewModel */ }
 *
 *     override fun onInitBind(binding: LayoutProfileCardBinding) {
 *         binding.tvName.text = "Initial Name"
 *         binding.btnEdit.setOnClickListener { viewModel.onEditClick() }
 *     }
 *
 *     override fun onEventVmCollect(binding: LayoutProfileCardBinding) {
 *         // Collect ViewModel events here
 *     }
 * }
 *
 * // 2. Use in XML
 * // <com.example.ProfileCardLayout
 * //     android:layout_width="match_parent"
 * //     android:layout_height="wrap_content" />
 *
 * // 3. Or create programmatically
 * // val profileCard = ProfileCardLayout(context)
 * // parentView.addView(profileCard)
 * ```
 * <br>
 *
 ...
 *
 * @see BaseDataBindingConstraintLayout For DataBinding variant with LiveData support.<br><br>
 *      LiveData 지원이 있는 DataBinding 버전은 BaseDataBindingConstraintLayout을 참조하세요.<br>
 */
abstract class BaseViewBindingConstraintLayout<BINDING : ViewBinding> : ParentsBindingConstraintLayout<BINDING> {
    /**
