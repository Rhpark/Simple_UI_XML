package kr.open.library.simple_ui.xml.ui.layout
...
/**
 * A FrameLayout that automatically observes the lifecycle of its hosting Activity or Fragment.<br>
 * Implements DefaultLifecycleObserver to receive lifecycle callbacks.<br><br>
 * 호스팅 Activity 또는 Fragment의 생명주기를 자동으로 관찰하는 FrameLayout입니다.<br>
 ...
 * Usage example:<br>
 * ```kotlin
 * class CustomFrameLayout(context: Context) : BaseLifeCycleFrameLayout(context) {
 ...
 *
 * @see BaseLifeCycleRelativeLayout For RelativeLayout with lifecycle support.<br><br>
 *      생명주기 지원이 있는 RelativeLayout은 BaseLifeCycleRelativeLayout을 참조하세요.<br>
 *
 ...
 */
class BaseLifeCycleFrameLayout : FrameLayout, DefaultLifecycleObserver {
