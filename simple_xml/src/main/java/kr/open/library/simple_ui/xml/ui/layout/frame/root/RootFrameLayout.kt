package kr.open.library.simple_ui.xml.ui.layout.frame.root
...
/**
 * FrameLayout that observes the host Activity or Fragment lifecycle.<br>
 ...
 * 호스트 Activity 또는 Fragment의 생명주기를 관찰하는 FrameLayout입니다.<br>
 * attach 시 자동으로 바인딩하고 detach 시 자동으로 해제합니다.<br>
 * EditMode 가드는 적용하지 않으며, 프리뷰 처리는 호출자 책임입니다.<br>
 */
abstract class RootFrameLayout :
    FrameLayout,
    ...
    private val lifecycleCoordinator = LayoutLifecycleCoordinator(
        ...
        callbacks = object : LayoutLifecycleCallbacks {
            override fun bindLifecycle(): LifecycleOwner? = bindLifecycleObserver(this@RootFrameLayout)

            override fun unbindLifecycle() = unbindLifecycleObserver(this@RootFrameLayout)
        },
    )
