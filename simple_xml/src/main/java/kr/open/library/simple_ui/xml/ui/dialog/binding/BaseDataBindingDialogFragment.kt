 ...
 * - DataBinding enables automatic UI updates when LiveData/StateFlow values change, reducing boilerplate update code.<br>
 ...
 * - DataBinding은 LiveData/StateFlow 값이 변경될 때 자동 UI 업데이트를 가능하게 하여 보일러플레이트 업데이트 코드를 줄입니다.<br>
 ...
 * - Automatically sets binding.lifecycleOwner in createBinding() to enable LiveData observation without manual setup.<br>
 * - Implements final createBinding() to prevent subclasses from breaking the lifecycleOwner initialization contract.<br>
 ...
 * - 수동 설정 없이 LiveData 관찰을 가능하게 하기 위해 createBinding()에서 자동으로 binding.lifecycleOwner를 설정합니다.<br>
 * - final createBinding()을 구현하여 하위 클래스가 lifecycleOwner 초기화 계약을 깨는 것을 방지합니다.<br>
 * - LiveData 옵저버로 인한 메모리 누수를 방지하기 위해 onDestroyView()에서 unbind()를 호출하고 lifecycleOwner를 null로 설정합니다.<br>
 *
 * **Important notes / 주의사항:**<br>
 * - ⚠️ CRITICAL: lifecycleOwner is automatically set to this DialogFragment in createBinding(). Unlike Activity's BaseDataBindingActivity, you don't need to set it manually.<br>
 * - DataBinding automatically updates UI when LiveData/StateFlow values change - no need for manual observe() calls in XML-bound properties.<br>
 ...
 * - ⚠️ 중요: lifecycleOwner는 createBinding()에서 이 DialogFragment로 자동 설정됩니다. Activity의 BaseDataBindingActivity와 달리 수동으로 설정할 필요가 없습니다.<br>
 * - DataBinding은 LiveData/StateFlow 값이 변경될 때 자동으로 UI를 업데이트합니다 - XML에 바인딩된 프로퍼티에 대해 수동 observe() 호출이 필요하지 않습니다.<br>
...
public abstract class BaseDataBindingDialogFragment<BINDING : ViewDataBinding>(
     ...
     * Automatically sets lifecycleOwner to this DialogFragment to enable LiveData observation.<br><br>
     * DataBindingUtil을 사용하여 DataBinding 인스턴스를 생성합니다.<br>
     * LiveData 관찰을 활성화하기 위해 자동으로 lifecycleOwner를 이 DialogFragment로 설정합니다.<br>
     *
     ...
     *                         부모에 첨부할지 여부.<br>
     * @return The initialized DataBinding instance with lifecycleOwner already set.<br><br>
     *         lifecycleOwner가 이미 설정된 초기화된 DataBinding 인스턴스.<br>
     */
    final override fun createBinding(inflater: LayoutInflater, container: ViewGroup?, isAttachToParent: Boolean): BINDING =
        DataBindingUtil.inflate<BINDING>(inflater, layoutRes, container, isAttachToParent).apply {
            lifecycleOwner = this@BaseDataBindingDialogFragment
        }
