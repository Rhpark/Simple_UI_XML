...
/**
 * LinearLayout that uses ViewBinding with a provided inflate function.<br><br>
 * 제공된 inflate 함수로 ViewBinding을 사용하는 LinearLayout입니다.<br>
 *
 * **Why this class exists / 이 클래스가 필요한 이유:**<br>
 * - Provides lifecycle-aware ViewBinding for custom LinearLayout components.<br>
 * - Ideal for toolbar components, list items, or vertically/horizontally stacked views.<br>
 * - Automatically initializes binding on attach and cleans up on detach.<br><br>
 * - 커스텀 LinearLayout 컴포넌트에 생명주기 인식 ViewBinding을 제공합니다.<br>
 * - 툴바 컴포넌트, 리스트 아이템, 또는 수직/수평 스택 뷰에 적합합니다.<br>
 ...
 * ) : BaseViewBindingLinearLayout<LayoutSettingsItemBinding>(
 *     context, attrs,
 *     inflate = LayoutSettingsItemBinding::inflate,
 * ) {
 *     init { orientation = HORIZONTAL }
 *
 *     override fun onInitBind(binding: LayoutSettingsItemBinding) {
 ...
 *
 * @see BaseDataBindingLinearLayout For DataBinding variant with LiveData support.<br><br>
 *      LiveData 지원이 있는 DataBinding 버전은 BaseDataBindingLinearLayout을 참조하세요.<br>
 */
abstract class BaseViewBindingLinearLayout<BINDING : ViewBinding> : ParentsBindingLinearLayout<BINDING> {
    /**
