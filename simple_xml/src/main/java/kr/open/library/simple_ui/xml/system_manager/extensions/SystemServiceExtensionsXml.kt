 ...
 * - Use `clearSystemBarControllerCache()` if you need to force recreate the controller.<br><br>
 * - 컨트롤러는 자동으로 파괴되지 않습니다. 사용 완료 시 수동으로 `onDestroy()`를 호출해야 합니다.<br>
 * - Activity/Dialog의 `onDestroy()` 라이프사이클 메서드에서 `onDestroy()` 호출을 권장합니다.<br>
 * - 컨트롤러를 강제로 재생성해야 하는 경우 `clearSystemBarControllerCache()`를 사용하세요.<br>
 *
 * **Usage / 사용법:**<br>
 * ```kotlin
...
/**
 * Clears the cached SystemBarController instance from this Window.<br>
 * After calling this, the next `getSystemBarController()` call will create a new instance.<br><br>
 * 이 Window에서 캐시된 SystemBarController 인스턴스를 제거합니다.<br>
 * 호출 후 다음 `getSystemBarController()` 호출 시 새 인스턴스가 생성됩니다.<br>
 *
 * **Important / 주의사항:**<br>
 * - This does NOT call `onDestroy()` on the cached controller.<br>
 * - You must manually call `onDestroy()` before clearing the cache to prevent memory leaks.<br>
 * - Typical usage: call `controller.onDestroy()` first, then `clearSystemBarControllerCache()`.<br><br>
 * - 이 메서드는 캐시된 컨트롤러의 `onDestroy()`를 호출하지 않습니다.<br>
 * - 메모리 누수를 방지하려면 캐시를 지우기 전에 수동으로 `onDestroy()`를 호출해야 합니다.<br>
 * - 일반적인 사용법: 먼저 `controller.onDestroy()` 호출 후 `clearSystemBarControllerCache()` 호출.<br>
 *
 * **Usage / 사용법:**<br>
 * ```kotlin
 * // Proper cleanup
 ...
 * window.clearSystemBarControllerCache()
 *
 * // Next call creates new instance
 * val newController = window.getSystemBarController()
 * ```<br><br>
 */
public fun Window.clearSystemBarControllerCache() {
    decorView.setTag(R.id.tag_system_bar_controller, null)
}
