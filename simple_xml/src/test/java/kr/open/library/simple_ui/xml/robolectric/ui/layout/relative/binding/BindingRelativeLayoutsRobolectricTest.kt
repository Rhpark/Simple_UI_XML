package kr.open.library.simple_ui.xml.robolectric.ui.layout.relative.binding
...
import kr.open.library.simple_ui.xml.ui.layout.relative.binding.BaseDataBindingRelativeLayout
import kr.open.library.simple_ui.xml.ui.layout.relative.binding.BaseViewBindingRelativeLayout
import kr.open.library.simple_ui.xml.ui.layout.relative.binding.ParentsBindingRelativeLayout
...
class BindingRelativeLayoutsRobolectricTest {
    @Test
    fun parentsBindingRelativeLayout_clearsBindingOnDetach() {
        val env = createEnv()
        val view = TestParentsBindingRelativeLayout(env.activity, clearBinding = true)
    ...
    @Test
    fun parentsBindingRelativeLayout_reusesBindingWhenClearFalse() {
        val env = createEnv()
        val view = TestParentsBindingRelativeLayout(env.activity, clearBinding = false)
    ...
    @Test
    fun baseViewBindingRelativeLayout_usesInflateFunction() {
        val env = createEnv()
        val recorder = InflateRecorder()
        val view = TestBaseViewBindingRelativeLayout(env.activity, recorder, attachToParent = false)
    ...
    @Test
    fun baseDataBindingRelativeLayout_setsLifecycleOwnerAndUnbinds() {
        val env = createEnv()
        mockStatic(DataBindingUtil::class.java).use { mocked ->
            ...
            val view = TestBaseDataBindingRelativeLayout(env.activity, layoutId = 123)
            attachAndLayout(env, view)
    ...
    private class TestParentsBindingRelativeLayout(
        context: Context,
        clearBinding: Boolean,
    ) : ParentsBindingRelativeLayout<TestViewBinding>(context) {
    ...
    private class TestBaseViewBindingRelativeLayout(
        context: Context,
        private val recorder: InflateRecorder,
        attachToParent: Boolean,
    ) : BaseViewBindingRelativeLayout<TestViewBinding>(
            context,
    ...
    private class TestBaseDataBindingRelativeLayout(
        context: Context,
        layoutId: Int,
    ) : BaseDataBindingRelativeLayout<ViewDataBinding>(context, layoutId) {
        var initCount = 0
        var eventCount = 0
