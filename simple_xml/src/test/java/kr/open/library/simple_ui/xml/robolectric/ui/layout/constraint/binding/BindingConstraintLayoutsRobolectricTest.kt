package kr.open.library.simple_ui.xml.robolectric.ui.layout.constraint.binding
...
import kr.open.library.simple_ui.xml.ui.layout.constraint.binding.BaseDataBindingConstraintLayout
import kr.open.library.simple_ui.xml.ui.layout.constraint.binding.BaseViewBindingConstraintLayout
import kr.open.library.simple_ui.xml.ui.layout.constraint.binding.ParentsBindingConstraintLayout
...
class BindingConstraintLayoutsRobolectricTest {
    @Test
    fun parentsBindingConstraintLayout_clearsBindingOnDetach() {
        val env = createEnv()
        val view = TestParentsBindingConstraintLayout(env.activity, clearBinding = true)
    ...
    @Test
    fun parentsBindingConstraintLayout_reusesBindingWhenClearFalse() {
        val env = createEnv()
        val view = TestParentsBindingConstraintLayout(env.activity, clearBinding = false)
    ...
    @Test
    fun baseViewBindingConstraintLayout_usesInflateFunction() {
        val env = createEnv()
        val recorder = InflateRecorder()
        val view = TestBaseViewBindingConstraintLayout(env.activity, recorder, attachToParent = false)
    ...
    @Test
    fun baseDataBindingConstraintLayout_setsLifecycleOwnerAndUnbinds() {
        val env = createEnv()
        mockStatic(DataBindingUtil::class.java).use { mocked ->
            ...
            val view = TestBaseDataBindingConstraintLayout(env.activity, layoutId = 123)
            attachAndLayout(env, view)
    ...
    private class TestParentsBindingConstraintLayout(
        context: Context,
        clearBinding: Boolean,
    ) : ParentsBindingConstraintLayout<TestViewBinding>(context) {
    ...
    private class TestBaseViewBindingConstraintLayout(
        context: Context,
        private val recorder: InflateRecorder,
        attachToParent: Boolean,
    ) : BaseViewBindingConstraintLayout<TestViewBinding>(
            context,
    ...
    private class TestBaseDataBindingConstraintLayout(
        context: Context,
        layoutId: Int,
    ) : BaseDataBindingConstraintLayout<ViewDataBinding>(context, layoutId) {
        var initCount = 0
        var eventCount = 0
