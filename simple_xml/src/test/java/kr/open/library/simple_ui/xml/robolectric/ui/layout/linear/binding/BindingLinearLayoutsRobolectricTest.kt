package kr.open.library.simple_ui.xml.robolectric.ui.layout.linear.binding
...
import kr.open.library.simple_ui.xml.ui.layout.linear.binding.BaseDataBindingLinearLayout
import kr.open.library.simple_ui.xml.ui.layout.linear.binding.BaseViewBindingLinearLayout
import kr.open.library.simple_ui.xml.ui.layout.linear.binding.ParentsBindingLinearLayout
...
class BindingLinearLayoutsRobolectricTest {
    @Test
    fun parentsBindingLinearLayout_clearsBindingOnDetach() {
        val env = createEnv()
        val view = TestParentsBindingLinearLayout(env.activity, clearBinding = true)
    ...
    @Test
    fun parentsBindingLinearLayout_reusesBindingWhenClearFalse() {
        val env = createEnv()
        val view = TestParentsBindingLinearLayout(env.activity, clearBinding = false)
    ...
    @Test
    fun baseViewBindingLinearLayout_usesInflateFunction() {
        val env = createEnv()
        val recorder = InflateRecorder()
        val view = TestBaseViewBindingLinearLayout(env.activity, recorder, attachToParent = false)
    ...
    @Test
    fun baseDataBindingLinearLayout_setsLifecycleOwnerAndUnbinds() {
        val env = createEnv()
        mockStatic(DataBindingUtil::class.java).use { mocked ->
            ...
            val view = TestBaseDataBindingLinearLayout(env.activity, layoutId = 123)
            attachAndLayout(env, view)
    ...
    private class TestParentsBindingLinearLayout(
        context: Context,
        clearBinding: Boolean,
    ) : ParentsBindingLinearLayout<TestViewBinding>(context) {
    ...
    private class TestBaseViewBindingLinearLayout(
        context: Context,
        private val recorder: InflateRecorder,
        attachToParent: Boolean,
    ) : BaseViewBindingLinearLayout<TestViewBinding>(
            context,
    ...
    private class TestBaseDataBindingLinearLayout(
        context: Context,
        layoutId: Int,
    ) : BaseDataBindingLinearLayout<ViewDataBinding>(context, layoutId) {
        var initCount = 0
        var eventCount = 0
