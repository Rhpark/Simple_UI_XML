...
import kr.open.library.simpleui_xml.temp.multi.viewbinding.activity.TempAdapterListMultiActivity
import kr.open.library.simpleui_xml.temp.multi.viewbinding.activity.TempAdapterRcvMultiActivity

/**
 * Menu activity that routes to temp adapter example screens.<br><br>
 * temp 어댑터 예제 화면으로 이동하는 메뉴 Activity입니다.<br>
 */
class TempAdapterExampleActivity : BaseViewBindingActivity<ActivityTempAdapterMenuBinding>(ActivityTempAdapterMenuBinding::inflate) {
    /**
    ...
    override fun onCreate(binding: ActivityTempAdapterMenuBinding, savedInstanceState: Bundle?) {
        binding.tvMenuTitle.text = "Temp Adapter Examples (ViewBinding)"
