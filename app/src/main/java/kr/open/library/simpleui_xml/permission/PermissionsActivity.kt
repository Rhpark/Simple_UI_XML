 ...
 * - 특수 권한만 요청: "다른 앱 위에 표시"를 OFF로 한 뒤 SYSTEM_ALERT_WINDOW를 요청하고, 설정 화면에서 돌아온 후 onDeniedResult 호출 여부를 확인합니다.<br>
 * - 런타임+특수 조합: WRITE_EXTERNAL_STORAGE는 허용된 상태로 두고 SYSTEM_ALERT_WINDOW를 OFF로 한 뒤 둘 다 요청한 다음, 설정 화면에서 돌아온 후에만 콜백이 호출되는지 확인합니다.<br>
 * - 홈 이동 후 복귀: 설정 화면으로 이동한 뒤 홈으로 나갔다가 앱으로 복귀했을 때 복귀 시점 상태가 반영되는지 확인합니다.<br>
 * - 허용 후 복귀: 설정에서 SYSTEM_ALERT_WINDOW를 허용한 뒤 복귀했을 때 deniedResults가 비어있는지 확인합니다.<br>
 */
