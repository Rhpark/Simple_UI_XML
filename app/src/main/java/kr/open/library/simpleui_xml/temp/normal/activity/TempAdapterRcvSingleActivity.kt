...
import kr.open.library.simpleui_xml.temp.normal.adapter.recyclerview.TempSimpleSingleNormalAdapter
import kr.open.library.simpleui_xml.temp.normal.adapter.recyclerview.TempSingleNormalAdapter
 ...
 */
class TempAdapterRcvSingleActivity : BaseActivity(R.layout.activity_temp_adapter_example) {
    /**
     ...
     */
    private val screenTitle: String = "Adapter - Single"

    /**
     * Whether the Simple adapter option is allowed.<br><br>
     * Simple 어댑터 옵션 사용 여부입니다.<br>
     */
    private val allowSimpleAdapter: Boolean = true
    ...
    private fun createAdapter(): RecyclerView.Adapter<*> {
        ...
        val enableDiffUtil = swEnableDiffUtil.isChecked
        val useSimpleAdapter = allowSimpleAdapter && swUseSimpleAdapter.isChecked

        return if (useSimpleAdapter) {
            TempSimpleSingleNormalAdapter(
                diffUtilEnabled = enableDiffUtil,
    ...
    /**
     * Initializes view references.<br><br>
     * 뷰 참조를 초기화합니다.<br>
     */
    ...
    private fun setupListeners() {
        btnResetItems.setOnClickListener { resetItems() }
        btnSetLargeItems.setOnClickListener { setLargeItems() }
        btnAddItem.setOnClickListener { addItem() }
        btnAddItemAt.setOnClickListener { addItemAt() }
        btnAddItems.setOnClickListener { addItems() }
        btnRemoveItem.setOnClickListener { removeItem() }
        btnRemoveAt.setOnClickListener { removeAt() }
        btnMoveItem.setOnClickListener { moveItem() }
        btnReplaceItem.setOnClickListener { replaceItem() }
        btnRemoveAll.setOnClickListener { removeAll() }
    ...
    private fun configureOptionAvailability() {
        ...
        swEnableDiffUtil.isEnabled = isAdapter
        swEnableDiffUtil.alpha = if (isAdapter) 1f else 0.5f
        if (!isAdapter) {
            swEnableDiffUtil.isChecked = false
        }

        swUseCustomDiffCallback.isEnabled = isList
        swUseCustomDiffCallback.alpha = if (isList) 1f else 0.5f
        if (!isList) {
            swUseCustomDiffCallback.isChecked = false
        }
    ...
    private fun submitInitialItems() {
        val adapter = requireAdapter() ?: return
        val items = TempItemGenerator.generateDefaultItemsSingle()
    ...
    private fun setLargeItems() {
        val adapter = requireAdapter() ?: return
        val items = TempItemGenerator.generateLargeItemsSingle()
        updateNextItemId(items)
        if (!withRootAdapter(
                adapter = adapter,
                onRcv = { rcvAdapter ->
    ...
    private fun createNewItem(label: String): TempItem {
        val item = TempItemGenerator.generateSingleItemSingle(nextItemId.toInt())
        nextItemId += 1
