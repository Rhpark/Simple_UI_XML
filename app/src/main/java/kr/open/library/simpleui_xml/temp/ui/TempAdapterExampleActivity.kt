...
/**
 * Activity that showcases temp adapter examples with operations and options.<br><br>
 * 예제 어댑터 옵션과 연산을 보여주는 액티비티입니다.<br>
 */
class TempAdapterExampleActivity : AppCompatActivity() {
    ...
    /**
     * Button to show previous example.<br><br>
     * 이전 예제를 표시하는 버튼입니다.<br>
     */
    private lateinit var btnPrevExample: Button

    /**
     * Button to show next example.<br><br>
     * 다음 예제를 표시하는 버튼입니다.<br>
     */
    private lateinit var btnNextExample: Button
    ...
    /**
     * Current example index.<br><br>
     * 현재 예제 인덱스입니다.<br>
     */
    private var currentIndex: Int = 0

    /**
     * Current example descriptor.<br><br>
     * 현재 예제 설명자입니다.<br>
     */
    private var currentExample: TempAdapterExample? = null
    ...
    private fun bindViews() {
        ...
        btnPrevExample = findViewById(R.id.btnPrevExample)
    ...
    private fun setupListeners() {
        ...
        swUseDiffExecutor.setOnCheckedChangeListener { _, _ -> showExample(currentIndex) }
        swEnableDiffUtil.setOnCheckedChangeListener { _, _ -> showExample(currentIndex) }
        swUseCustomDiffCallback.setOnCheckedChangeListener { _, _ -> showExample(currentIndex) }
    }

    /**
     * Shows the example at the given index, wrapping around bounds.<br><br>
     * 지정 인덱스의 예제를 표시하며 범위를 넘어가면 순환합니다.<br>
     */
    private fun showExample(index: Int) {
        if (TempAdapterExamples.all.isEmpty()) {
            updateStatus("Example list is empty")
            return
        }

        // Normalized index with wrapping.<br><br>범위를 순환한 정규화 인덱스입니다.<br>
        val normalizedIndex =
            when {
        ...
        // Current example descriptor.<br><br>현재 예제 설명자입니다.<br>
        val example = TempAdapterExamples.all[currentIndex]
        currentExample = example

        tvExampleTitle.text = example.title
        updateOptionAvailability(example.kind)

        // Dependencies built from current options.<br><br>현재 옵션으로 생성된 의존성입니다.<br>
        val dependencies = buildDependencies(example.kind)
        // Adapter created for the example.<br><br>예제용으로 생성된 어댑터입니다.<br>
        val adapter = example.createAdapter(dependencies)
        rcvTempExample.adapter = adapter
        applyQueuePolicy(adapter)
        bindAdapterListeners(adapter)

        // Initial items for the example.<br><br>예제 초기 아이템 목록입니다.<br>
        val items = createItemsForMode(example.itemMode, DEFAULT_ITEM_COUNT)
        updateNextItemId(items)
        submitItems(adapter, items, "setItems")
    }

    /**
     * Updates option availability based on adapter kind.<br><br>
     * 어댑터 종류에 따라 옵션 활성화를 업데이트합니다.<br>
     */
    private fun updateOptionAvailability(kind: TempAdapterKind) {
        swEnableDiffUtil.isEnabled = (kind == TempAdapterKind.RV)
        swUseCustomDiffCallback.isEnabled = (kind == TempAdapterKind.LIST)
    }
     ...
     */
    private fun buildDependencies(kind: TempAdapterKind): TempAdapterDependencies {
        // Optional diffExecutor injection.<br><br>선택적 diffExecutor 주입입니다.<br>
        val diffExecutor = if (swUseDiffExecutor.isChecked) diffExecutorService else null
        // Optional custom diff callback for list adapters.<br><br>ListAdapter용 커스텀 diff 콜백입니다.<br>
        val diffCallback =
            if (kind == TempAdapterKind.LIST && swUseCustomDiffCallback.isChecked) {
                TempItemDiffCallback()
            } else {
                null
            }
        // DiffUtil toggle for normal adapters only.<br><br>일반 어댑터용 DiffUtil 토글입니다.<br>
        val diffUtilEnabled = (kind == TempAdapterKind.RV && swEnableDiffUtil.isChecked)
    ...
    /**
     * Returns the current example or null if missing.<br><br>
     * 현재 예제를 반환하며 없으면 null을 반환합니다.<br>
     */
    private fun requireExample(): TempAdapterExample? {
        val example = currentExample
        if (example == null) {
            updateStatus("Example is not ready")
        }
        return example
    }
    ...
    private fun handleResetItems() {
        val example = requireExample() ?: return
    ...
    private fun handleSetLargeItems() {
        val example = requireExample() ?: return
        val adapter = requireAdapter() ?: return
        val items = createItemsForMode(example.itemMode, LARGE_ITEM_COUNT)
        updateNextItemId(items)
    ...
    private fun handleAddItem() {
        val example = requireExample() ?: return
        val adapter = requireAdapter() ?: return
        val newItem = createNewItem(example.itemMode, "Add")
        val handled = withRootAdapter(
            adapter = adapter,
            onRcv = { rcvAdapter ->
    ...
    private fun handleAddItemAt() {
        val example = requireExample() ?: return
        val adapter = requireAdapter() ?: return
        val newItem = createNewItem(example.itemMode, "AddAt")
        val handled = withRootAdapter(
            adapter = adapter,
            onRcv = { rcvAdapter ->
    ...
    private fun handleAddItems() {
        ...
        val newItems = List(ADD_ITEMS_COUNT) { index ->
            ...
            createNewItem(example.itemMode, label)
        }
        val handled = withRootAdapter(
            adapter = adapter,
            onRcv = { rcvAdapter ->
