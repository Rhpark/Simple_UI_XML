...
`simple_system_manager` consolidates system manager controllers and extension entry points that were previously split between `simple_core` and `simple_xml`.

This page is a quick overview. See the feature-specific README and specification documents for detailed behavior.

> `simple_system_manager`는 기존 `simple_core`, `simple_xml`에 흩어져 있던 system manager controller와 확장 진입점을 한 모듈로 정리합니다.
>
> 이 문서는 빠른 개요용 요약 문서이며, 상세 동작은 하위 README와 기능 명세 문서를 참조합니다.
...
### Core-origin Controllers (Core 출신 Controller)
...
### XML-origin Controllers (XML 출신 Controller)
| Controller | Summary | Docs |
|---|---|---|
...
### Extension Entry Points (확장 진입점)

- Context/Window extension list: [README_SYSTEM_MANAGER_EXTENSIONS.md](../README_SYSTEM_MANAGER_EXTENSIONS.md)
- System bar entry points:
...
- Reduces repetitive `getSystemService()` calls and SDK-version branching.
- Provides a consistent controller pattern for system-service operations.
- Reuses common `simple_core` utilities while keeping public system manager APIs in `simple_system_manager`.
...
- Notifications on Android 13+ require the `POST_NOTIFICATIONS` permission.
- A `NotificationChannel` is required, and `createChannel()` affects only notifications created afterward.
- Call `cleanup()` when an Activity or Service that used progress notifications is being destroyed.
- Alarm-triggered notification paths also check `POST_NOTIFICATIONS` at runtime and skip notification display when permission is denied.
- `VIBRATE` is a normal permission, so only a manifest declaration is required.
- `removeAllFloatingView()` uses a `first-failure-stop` strategy, so a partial cleanup state can remain after a failure.
- `window.getSystemBarController()` and `window.destroySystemBarControllerCache()` have an `@MainThread` contract.
