...
3. **스케줄 기반 자동 선택 등록**
   - `AlarmScheduleData.idleMode`에 따라 등록 API 자동 선택
4. **날짜 지정 알람**
   - `AlarmDateData`로 특정 날짜 지정 및 유효성 검증
5. **알람 업데이트**
...
- **안정성**: 예외 발생 시 크래시 대신 안전한 실패(`SystemResult.Failure`/로그)로 처리. exact 권한 거부 시 `SystemResult.PermissionDenied`로 원인 구분
- **성능**: 브로드캐스트 처리에서 최소 작업 수행, 필요 시 WakeLock만 짧게 유지
- **유지보수성**: 스케줄/알림 DTO 분리 및 명확한 API 책임
- **호환성**: minSdk 28 기준, API 31+ 정확 알람 권한 대응
- **정합성**: AlarmClock의 UI 진입용 `showIntent`와 실제 알람 트리거 `PendingIntent`를 분리해 조기 브로드캐스트를 방지

## 제약/전제

- 알람 저장/복원은 앱 레벨에서 구현한다(`loadAllAlarmDataList`, `loadAlarmData`). → 비목표 "알람 저장소(DB/Preference) 구현 제공" 참조.
