...
- 기본 규칙/환경은 루트 AGENTS.md에서 연결되는 docs/rules/CODING_RULE_INDEX.md를 따른다.
...
- `controller/alarm`
  ...
  - `vo/AlarmData` 및 하위 DTO: 스케줄/알림 데이터 모델
...
- 필드
  ...
  - `date: AlarmDateData?` (없으면 매일 기준, 있으면 특정 날짜)
- 검증
...
- 필드
  ...
  - `acquireTime: Long` (기본 `AlarmConstants.DEFAULT_ACQUIRE_TIME_MS`, deprecated)
    - 런타임에서 알람별 WakeLock 시간으로 반영되지 않음
    - 실제 WakeLock 정책은 `BaseAlarmReceiver.powerManagerAcquireTime` 사용
- 유틸
...
public open class AlarmController(context: Context) : BaseSystemService {
    ...
    fun registerAlarmClock(receiver: Class<*>, alarmData: AlarmData, namespace: String? = null): SystemResult<Unit>

    @RequiresPermission(SCHEDULE_EXACT_ALARM, conditional = true)
    fun registerAlarmExactAndAllowWhileIdle(receiver: Class<*>, alarmData: AlarmData, namespace: String? = null): SystemResult<Unit>

    fun registerAlarmAndAllowWhileIdle(receiver: Class<*>, alarmData: AlarmData, namespace: String? = null): SystemResult<Unit>
    ...
    fun registerBySchedule(receiver: Class<*>, alarmData: AlarmData, namespace: String? = null): SystemResult<Unit>

    fun updateAlarmClock(receiver: Class<*>, alarmData: AlarmData, namespace: String? = null): SystemResult<Unit>
    fun updateExactAndAllowWhileIdle(receiver: Class<*>, alarmData: AlarmData, namespace: String? = null): SystemResult<Unit>
    fun updateAllowWhileIdle(receiver: Class<*>, alarmData: AlarmData, namespace: String? = null): SystemResult<Unit>
    fun updateRepeating(
        receiver: Class<*>,
        alarmData: AlarmData,
        intervalMillis: Long,
    ...
    fun remove(key: Int, receiver: Class<*>, namespace: String? = null): SystemResult<Boolean>
...
- 등록/갱신/제거/존재 확인 함수는 `tryCatchSystemManagerResult`로 감싸져 있으며, 예외 발생 시 `SystemResult.Failure`를 반환한다.
- exact 계열(`registerAlarmClock`, `registerAlarmExactAndAllowWhileIdle`)은 `tryCatchSystemManagerResult` 진입 전 `canScheduleExactAlarms()` 체크를 수행하며, 권한 거부 시 `SystemResult.PermissionDenied`를 반환한다.
- inexact 계열(`registerAlarmAndAllowWhileIdle`, `registerRepeating`)은 권한 체크 없이 동작한다.
- `namespace`는 requestCode 충돌 방지용이며, 같은 알람에 대해 **등록/삭제/조회 시 동일한 값**을 사용해야 한다.
- `AlarmConstants.ALARM_KEY`는 PendingIntent에 반드시 삽입된다.

#### registerAlarmClock

- API 31+에서 `canScheduleExactAlarms()`가 false면 `SystemResult.PermissionDenied` 반환 및 경고 로그 출력
- 권한 체크는 `tryCatchSystemManagerResult` 진입 전에 수행된다
- `getCalendar(alarmData)`로 시간 계산
- trigger용 Broadcast `PendingIntent`와 `AlarmClockInfo.showIntent`용 Activity `PendingIntent`를 분리한다
- `AlarmClockInfo.showIntent`는 앱 launch intent를 우선 사용하고, 없으면 앱 상세 설정 화면 intent를 사용한다
- PendingIntent 생성 실패 시 `SystemResult.Failure(null)` 반환
- `AlarmManager.setAlarmClock()` 호출

#### registerAlarmExactAndAllowWhileIdle

- API 31+에서 `canScheduleExactAlarms()` 체크(거부 시 `SystemResult.PermissionDenied` 반환)
- 권한 체크는 `tryCatchSystemManagerResult` 진입 전에 수행된다
...
- `AlarmScheduleData.idleMode` 기반으로 등록 API 선택
...
#### getCalendar(alarmData)

- `AlarmScheduleData.date`가 **없으면**:
  - 오늘 시간이 지났으면 다음 날로 보정
- `AlarmScheduleData.date`가 **있으면**:
...
abstract class BaseAlarmReceiver : BroadcastReceiver() {
    ...
    protected open fun resolveAlarmNamespace(alarmData: AlarmData): String?
    protected abstract fun createNotificationChannel(context: Context, notification: AlarmNotificationData)
    protected abstract fun buildNotificationOption(context: Context, alarmData: AlarmData): SimpleNotificationOptionBase
    ...
    protected abstract fun loadAlarmData(context: Context, intent: Intent, key: Int): AlarmData?
...
- `loadAllAlarmDataList()`는 **영속 저장소**(DB/파일/Preference) 기반으로 구현해야 한다.
- `loadAlarmData()`는 key 기반 단건 로드를 수행해야 한다.

## 오류 처리/로그 정책

- AlarmController:
  - exact 권한 거부 시 `SystemResult.PermissionDenied` 반환
  - 예외 발생 시 `SystemResult.Failure` 반환
  - Logx로 경고/디버그 로그 출력
