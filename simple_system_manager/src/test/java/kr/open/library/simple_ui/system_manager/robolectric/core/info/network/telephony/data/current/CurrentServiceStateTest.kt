package kr.open.library.simple_ui.system_manager.robolectric.core.info.network.telephony.data.current
...
class CurrentServiceStateTest {
    @Test
    fun `accessors return null for invalid indexes and missing service state`() {
        val serviceState = mock(ServiceState::class.java)
        doReturn(emptyList<NetworkRegistrationInfo>()).`when`(serviceState).networkRegistrationInfoList

        val empty = CurrentServiceState(serviceState)
        val missing = CurrentServiceState(null)

        assertNull(empty.getNetworkRegistrationInfo(-1))
        assertNull(empty.getNetworkRegistrationInfo(0))
        assertNull(empty.getDomain(0))
        assertNull(empty.getTransportType(0))
        assertNull(empty.getAccessNetworkTechnology(0))
        assertNull(empty.getAvailableServices(0))
        assertNull(missing.getNetworkRegistrationInfo(0))
    }
