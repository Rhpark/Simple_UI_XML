...
## 수정 전 필수 프리체크
 - 분석 요약을 5~15줄로 먼저 제시한다.
 - 변경 대상 파일 목록을 사전에 공유한다.
 - 관련 호출부 및 영향 범위를 사전에 명시한다.
 - UTF-8 형식 유지와 한글 깨짐 검수 계획을 사전에 명시한다.
 - 사용자 명시 승인 전에는 읽기/분석만 수행하고 파일 수정은 금지한다.
...
## README
 - README 파일 매핑은 docs/rules/README_RULE.md 를 참조한다.
