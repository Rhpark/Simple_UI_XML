...
SubAgent에게 위임하기 전 반드시 아래 순서로 수행한다.

1. 모듈 AGENTS.md 읽기 → SubAgent 유형 판단
2. 해당 기능의 PRD/SPEC 읽기 → 하네스 요소 추출 (문서가 있는 경우)
3. 아래 형식으로 위임 내용을 재구성하여 사용자에게 보고한다.

```md
SubAgent: {agent명}
대상: {파일/패키지 경로}
진입 단계: {STEP1부터 / STEP2부터 등}
하네스 컨텍스트:
  - 금지 패턴: {개수}개 — {핵심 내용 요약}
  - 경계 조건: {핵심 내용 요약}
  - 판단 기준: {핵심 내용 요약}
탐색 범위: {읽어야 할 파일/패키지 목록}
```

사용자 승인 후 SubAgent 실행

### 위임 프롬프트 필수 명시 항목

위임 시 프롬프트에 아래 5가지를 반드시 명시한다.

1. 목적: 무엇을 알아내야 하는가
2. 범위: 반드시 읽어야 할 파일/패키지 경로 (해당 모듈 AGENTS.md 포함)
3. 출력 형식: 어떤 구조로 보고할 것인가
4. 금지 사항: 코드 수정 불가, 추정으로 결론 내리지 않기 등
5. 하네스 문서: 해당 기능의 PRD/SPEC 경로 및 추출한 하네스 요소 (있는 경우)
...
아래 작업은 전용 Agent가 있다. 요청에 맞는 agent를 활용한다.
...
## 작업 도구 (Skills)
 ...
 - 코드 리뷰         : 우선순위 5단계(아키텍처→기능→로직→품질→명명) 순서로 점검, 100점 평가(리뷰 5단계와 1:1 정렬된 점수축)
...
## 기능 수정/분석 시 참조
 ...
 - 기능별 PRD/SPEC/IMPLEMENTATION_PLAN 경로는 각 모듈 AGENTS.md의 `기능별 전용 규칙` 섹션에서 확인한다.
 - 문서가 없는 경우 → `{모듈}/docs/feature/{기능명}/` 패턴으로 직접 탐색한다.
