...
아래 6가지 항목을 순서대로 점검한다. 문제가 발견되어도 모든 항목을 끝까지 수행한다.
...
아래 조건 중 하나에 해당하면 SubAgent(general-purpose)를 호출하여 분석을 위임한다.

| 조건 | 분석 내용 |
|---|---|
| 사용자가 명시적으로 중복 구조 검사를 요청한 경우 | SubAgent가 `res/layout/` 디렉터리 전체를 스캔하여 중복 뷰 구조 감지 → `<include>` / `<merge>` 활용 제안 |
| ConstraintLayout을 사용하는 파일인 경우 | 연결되지 않은 constraint, 누락된 constraint 감지 |
...
[SubAgent] ConstraintLayout 제약 분석  ← ConstraintLayout 사용 시에만 표시
  · {뷰ID} - {문제 내용} → {수정안}
```
