...
| Object (nullable, JSON 값이 null이지만 구조 존재) | `{ClassName}?` + `// TODO: nullable 중첩 객체 - 런타임에 null 가능성 있음. 초기화 로직 확인 필요` 주석 |
...
camelCase 변환 후 프로퍼티명이 Kotlin hard keyword와 충돌하는 경우 `{keyword}Field`로 변경한다.

**Kotlin hard keyword 목록:**
...
직렬화 라이브러리가 있는 경우 → `{keyword}Field`로 프로퍼티명 변경 + 어노테이션에 원본 키 보존
```kotlin
// 예: JSON 키 "class" → Gson 사용 시
@SerializedName("class")
val classField: String,
```

순수 Data Class인 경우 → `{keyword}Field`로 변경 + TODO 주석 추가
```kotlin
// 예: JSON 키 "class" → 라이브러리 없을 때
val classField: String, // TODO: 'class'는 Kotlin 예약어 - 원본 JSON 키와 이름이 다름. 직렬화 시 주의 필요
```

> 충돌 발생 시 STEP 3 계획 보고 및 STEP 7 결과 보고에 해당 필드를 명시한다.
...
- ⚠️ Kotlin 예약어 충돌: {원본키} → {keyword}Field 로 변경 예정  ← 예약어 충돌 발생 시에만 표시
- ⚠️ Long 타입 생성 예정: {필드명} (Int 범위 초과)  ← Long 타입이 생성될 경우에만 표시
- ⚠️ nullable 중첩 객체: {필드명}({ClassName}?) - JSON 값이 null  ← nullable 중첩 객체가 있는 경우에만 표시
```
...
- 생성 파일:
  · {경로} (클래스명, 필드 N개)
- ⚠️ TODO 항목: {필드명} - {사유}  ← 없으면 생략
- ⚠️ Kotlin 예약어 충돌: {원본키} → {keyword}Field 로 변경됨. 직렬화 시 원본 키 매핑 확인 필요  ← 예약어 충돌 발생 시에만 표시
- ⚠️ Long 타입 사용: {필드명}은 Int 범위(2,147,483,647)를 초과하여 Long으로 생성됨. 서버 응답값 범위가 변경될 경우 타입 검토 필요  ← Long 타입이 생성된 경우에만 표시
- ⚠️ nullable 중첩 객체: {필드명}({ClassName}?)은 JSON 값이 null이었음. 런타임 null 가능성 있으므로 초기화 로직 확인 필요  ← nullable 중첩 객체가 있는 경우에만 표시
