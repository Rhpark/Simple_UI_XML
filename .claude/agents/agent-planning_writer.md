...
### 하네스 요소 충분성 자가 점검
문서 작성 전, 도출한 하네스 요소가 충분한지 아래 기준으로 자가 점검한다.

**금지 패턴**
- 충분: 코드 작성 시 즉시 판단 가능한 수준
  - 충분 예: "Activity/Fragment 의존 로직을 core에 두지 않는다"
  - 불충분 예: "잘못된 패턴을 사용하지 않는다" → 어떤 파일/클래스에서 무엇이 금지인지 구체화 필요
- 미달 시: 어떤 파일·클래스에서 어떤 행동이 금지되는지 구체화한다

**판단 기준**
- 충분: "A vs B 선택" 형식이며, 새 코드 배치 위치를 즉시 결정 가능한 수준
  - 충분 예: "UI 이벤트 필요 → xml 모듈, 시스템 API만 사용 → core 모듈"
  - 불충분 예: "상황에 따라 결정한다" → 조건-결론 형식으로 재작성 필요
- 미달 시: 어떤 상황에서 어떤 선택을 하는지 조건-결론 형식으로 재작성한다

**경계 조건**
- 충분: 책임 범위와 비책임 범위가 각각 1개 이상 명시됨
  - 충분 예: "책임: 권한 요청 트리거 / 비책임: 권한 거부 시 UI 처리"
  - 불충분 예: "권한 관련 기능을 제공한다"만 있고 비책임 범위 없음 → 비책임 범위 추가 필요
- 미달 시: "이 기능이 하지 않는 것"을 명시적으로 추가한다
...
- 코드 분석 결과가 수집된 요구사항과 충돌하는 경우 (방향 확인 먼저)
- 하네스 요소 후보(금지 패턴/판단 기준/경계 조건)를 1개도 도출할 수 없는 경우
- 기능 범위가 승인 시점보다 크게 확장되어야 하는 경우
- 의존 모듈 범위가 불명확하여 문서 경계를 결정할 수 없는 경우
