...
- STEP6 (선택): [STEP6_SCORE.md](../../docs/agents/review/step6/STEP6_SCORE.md) — 사용자가 "평가해줘", "점수화해줘" 등을 명시한 경우만 실행
...
- 출력 규칙: [OUTPUT_RULE.md](common/OUTPUT_RULE.md) — 보고서 파일 저장 시 참조
- 상호작용 규칙: [INTERACTION_RULE.md](common/INTERACTION_RULE.md) — sub-agent 호출 시 사용자 질문·재진입 처리
...
3. 관련 모듈이 확정되면 [BASE_RULE.md](platform/android/common/BASE_RULE.md)를 읽어 해당 모듈 `AGENTS.md` 경로를 확인한다. <!-- ★ -->
4. 관련 모듈이 불명확하면 STEP1 진입 전에 사용자에게 먼저 확인한다.

## 시작 전 환경 파악 (필수)

리뷰 시작 전 반드시 [ENV_RULE.md](platform/android/common/ENV_RULE.md) 절차를 수행한다. <!-- ★ -->

## 진입 분기

STEP1을 시작하기 전에 아래 분기를 먼저 확인한다.

- **분기 D — 재진입**: 컨텍스트에 이전 STEP 산출물 블록이 있다 → [INTERACTION_RULE.md](common/INTERACTION_RULE.md) 재진입 동작을 따라 마지막 완료 STEP 다음부터 재개한다. 다른 분기보다 먼저 확인한다.
