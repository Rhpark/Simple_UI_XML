...
## 분석 중단 조건
아래 중 하나라도 해당하면 분석을 중단하고 사용자에게 보고한다.

- 리뷰 대상 파일이 존재하지 않는 경우
- 분석에 필요한 핵심 의존 파일 접근이 불가한 경우
- 리뷰 범위가 불명확하여 방향 결정이 필요한 경우 (범위 확인 후 재시작)
...
3. docs/rules/code_review/STEP3_ARCHITECTURE.md - 아키텍처
   ...
   - `{모듈}/docs/feature/{기능명}/PRD.md` 읽기 → 금지 패턴 / 경계 조건 위반 여부 확인
   - `{모듈}/docs/feature/{기능명}/SPEC.md` 읽기 → 판단 기준 / 내부 구조 배치 위반 여부 확인
   - 문서가 없는 경우 → 하네스 점검 생략, CODE_ARCHITECTURE.md 기준으로만 판단
   하네스 위반 심각도:
   - CRITICAL: PRD 금지 패턴 위반
   - HIGH: PRD 경계 조건 이탈
   - MEDIUM: SPEC 판단 기준 불일치 (코드 배치 위치 오류)
...
1. 이슈 목록을 아래 형식으로 요약 제공합니다.
   ...
   - [하네스 위반] PRD 금지 패턴 / PRD 경계 조건 / SPEC 판단 기준 — {문서 경로}
   ```
2. "이 이슈를 리팩토링하려면 agent-refactor에 위 요약을 전달하세요"를 안내합니다.
3. 하네스 위반 이슈가 있는 경우 관련 PRD/SPEC 경로도 함께 전달하도록 안내합니다.
4. 코드를 직접 수정하지 않습니다. (리뷰 에이전트는 읽기 전용)
