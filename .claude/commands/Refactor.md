docs/rules/coding_rule/CODE_PATTERNS_INDEX.md 의 금지 패턴 목록(필요 시 patterns/ 하위 상세 문서 포함)을 읽고,
...
2. CODE_PATTERNS_INDEX.md 에 정의된 금지 패턴 위반 항목을 파일별로 수집한다.
...
## SubAgent 결과 처리
1. SubAgent 결과를 직접 검토하여 교차 검증한다.
2. 전체 위반 목록을 사용자에게 고지하고 승인을 요청한다.
3. 승인 후 파일에 직접 반영한다.
4. 수정된 항목 목록과 성공/실패 여부를 요약해서 알려준다.
