...
class RolePermissionHandler(
     ...
     *
     * RoleManager는 Android Q(API 29) 이상에서만 지원되므로 미만 버전에서는 false를 반환합니다.<br><br>
     * RoleManager is only supported on Android Q (API 29) and above; returns false on lower versions.<br>
     *
     ...
     */
    fun isRoleAvailable(role: String): Boolean {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) return false
        return safeCatch(defaultValue = false) {
            context
                .getSystemService(RoleManager::class.java)
                ?.isRoleAvailable(role) ?: false
        }
    }
     ...
     *
     * RoleManager는 Android Q(API 29) 이상에서만 지원되므로 미만 버전에서는 null을 반환합니다.<br><br>
     * RoleManager is only supported on Android Q (API 29) and above; returns null on lower versions.<br>
     *
     ...
     */
    fun createRequestIntent(role: String): Intent? {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) return null
        return safeCatch(defaultValue = null) {
            context.getSystemService(RoleManager::class.java)?.createRequestRoleIntent(role)
        }
    }
}
