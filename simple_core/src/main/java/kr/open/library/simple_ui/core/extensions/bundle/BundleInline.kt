/**
 * Package containing Bundle extensions for safe, type-aware value access.<br><br>
 * 번들에서 타입 안전하게 값을 읽어오는 확장 함수를 모아 둔 패키지입니다.<br>
 */
 ...
 *
 * @param T The type to cast the value to.<br><br>
 *          값을 변환할 타입.
 ...
 * @return Either the stored value cast to `T` or [defaultValue] when unavailable.
 *         Logs an error via [Logx] when the key is missing or the requested type is unsupported.<br><br>
 *         저장된 값이 있다면 해당 타입의 값, 그렇지 않다면 [defaultValue].
 *         지원하지 않는 타입이거나 키를 찾지 못하면 [Logx]로 오류를 남깁니다.<br>
 */
